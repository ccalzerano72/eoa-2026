...
- **Fase D — Qualità & Docs**: COMPLETED.
  - Suite test minima a zero dipendenze (`npm test`, `tsx --test`, 32 test verdi): `generator/src/engine.test.ts` (evaluate/constraints/sampling/shuffle/distrattori/keywords/istanze 5 tipi), `app/src/data/study-links.test.ts` (resolver + regressione anti-ancore-morte contro i JSON teoria), `generator/src/archive-snapshot.test.ts` (snapshot archivio: totale, ID unici, bande distribuzione, Golden Rule, option set, double-unit).
  - `app/README.md` riscritto (stack, script, layout, convenzioni, quality gates).
  - `DESIGN_SPEC.md` §11 "Estensioni Approvate Post-M5" (tracciamento delta + decisioni: offline-first/M6 differito, telemetry opt-in, `BOOK/` untracked).
  - Decisioni proprietario registrate: offline-first/PWA NON prioritari (rinviati); `BOOK/` resta fuori git intenzionalmente.

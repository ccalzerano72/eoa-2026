...
import { TrendSparkline } from "./components/stats/charts";
...
export function App() {
  ...
  const applyNavEntry = useCallback(
    (entry: NavEntry) => {
      ...
      // Move keyboard focus to main content (a11y for SPA navigation)
      document
  ...
  const handleStartFreePractice = (questionCount = 15) => {
    const pool = filterByTrack(allQuestions);
    launchQuiz(pool, questionCount, "free-practice", "practice", {
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-900 text-slate-900 dark:text-slate-100 flex flex-col font-sans transition-colors">
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200 dark:border-slate-700 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md shadow-2xs">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-3 py-2 sm:px-6 sm:py-3">
          ...
          <div className="flex items-center gap-2 sm:gap-3 shrink-0">
            ...
            <div className="flex items-center gap-1 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl border border-slate-200/80 dark:border-slate-700">
              ...
              <button
                ...
                onClick={goForward}
                aria-label="Vai alla schermata successiva"
                className={`p-1.5 rounded-lg transition ${
                  canGoForward
                    ? "text-slate-700 dark:text-slate-200 hover:bg-white dark:hover:bg-slate-700 hover:text-slate-950 dark:hover:text-white shadow-2xs cursor-pointer"
                    : "text-slate-300 dark:text-slate-600 cursor-not-allowed"
            ...
            <div
              ...
              className="flex items-center gap-2 sm:gap-3 cursor-pointer select-none rounded-lg"
            >
          ...
          {/* Navigation links */}
          <nav className="flex items-center gap-0.5 sm:gap-2 shrink-0 flex-wrap justify-end">
            {/* Persistent active-track badge */}
            <button
              onClick={() => navigateTo({ tab: "dashboard" })}
              title="Percorso attivo — cambia in Modalità di Studio Avanzate"
              aria-label={`Percorso attivo: ${activeTrackFilter === "all" ? "tutti" : activeTrackFilter}. Vai alla dashboard per cambiarlo.`}
              className={`hidden min-[400px]:inline-flex items-center px-2 py-1 rounded-lg text-[11px] font-bold border transition cursor-pointer ${
            ...
            <div className="flex items-center bg-slate-100 dark:bg-slate-800 p-0.5 rounded-lg border border-slate-200 dark:border-slate-700">
              ...
              <button
                ...
                aria-label="Cambia tema chiaro/scuro/sistema"
              >
                {theme === "light" ? (
              ...
              <button
                ...
                title="Segui sistema"
                aria-label="Tema automatico dal sistema"
              >
      ...
      <main id="main-content" tabIndex={-1} className="flex-1 outline-none">
          ...
          <div className="mx-auto max-w-5xl py-10 px-4 sm:px-6">
            ...
            <div className="rounded-3xl bg-linear-to-br from-slate-900 via-sky-950 to-indigo-950 p-8 sm:p-10 text-white shadow-xl">
              ...
              <div className="mt-8 flex flex-col sm:flex-row gap-3 sm:gap-4">
                ...
                <button
                  ...
                  className="inline-flex items-center justify-center gap-2 rounded-xl bg-white/10 hover:bg-white/20 border border-emerald-300/40 px-6 py-3 font-bold text-sm text-white transition cursor-pointer w-full sm:w-auto"
                >
                  <Target className="h-5 w-5 shrink-0" />
                  <span>Pratica Libera (15 quesiti)</span>
                </button>
                <button
            ...
            {overallStats.totalSessions > 0 ? (
              ...
              >
                ...
                {/* Per-block mastery bars */}
                <div className="mt-4 space-y-1.5 text-left">
                  {[1, 2, 3, 4, 5, 6, 7].map((bNum) => {
                    const bStats =
                      overallStats.blockStats[bNum as SyllabusBlock];
                    const has = bStats.totalQuestions > 0;
                    const acc = bStats.accuracyPercentage;
                    return (
                      <div key={bNum} className="flex items-center gap-2">
                        <span className="w-6 shrink-0 font-mono text-[11px] font-bold text-slate-500 dark:text-slate-400">
                          B{bNum}
                        </span>
                        <div className="h-2 flex-1 rounded-full bg-slate-200 dark:bg-slate-700 overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${has ? Math.max(acc, 5) : 0}%`,
                            }}
                          />
                        </div>
                        <span className="w-12 shrink-0 text-right text-[11px] font-bold text-slate-600 dark:text-slate-300">
                          {has ? `${acc}%` : "—"}
                        </span>
                      </div>
                    );
                ...
                {examTrend.length > 1 && (
                  <div className="mt-4 flex flex-col items-center gap-1">
                    <TrendSparkline points={examTrend} width={240} height={44} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                {[
                  ...
                  <button
                    ...
                    aria-label={`Apri il Blocco ${b.num}: ${b.title}`}
                    className={`rounded-2xl border bg-white dark:bg-slate-800 p-5 shadow-2xs hover:shadow-md transition cursor-pointer border-l-4 text-left w-full ${b.color}`}
                  >
                ...
                >
                  ...
                  {spacedRepStats.totalTracked > 0 ? (
                    <div className="flex items-center gap-2 text-[11px]">
                      <span className="px-2 py-0.5 rounded-full bg-rose-100 dark:bg-rose-900/50 text-rose-700 dark:text-rose-300 font-semibold">
                      ...
                      <span className="px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 font-semibold">
            ...
            <CollapsibleSection
              ...
              badge={
                <span className="hidden sm:inline text-xs font-mono font-semibold px-2.5 py-1 rounded-full bg-purple-100 dark:bg-purple-900/50 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800">
                  NUOVO
                </span>
              }
              defaultExpanded={false}
                  ...
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="flex flex-wrap gap-2 text-xs">
                        <span className="px-2 py-1 rounded-lg bg-emerald-100 dark:bg-emerald-900/50 text-emerald-700 dark:text-emerald-300 font-semibold">
                          Leva Finanziaria
                        </span>
                        <span className="px-2 py-1 rounded-lg bg-sky-100 dark:bg-sky-900/50 text-sky-700 dark:text-sky-300 font-semibold">
                          Modigliani-Miller
                        </span>
                        <span className="px-2 py-1 rounded-lg bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300 font-semibold">
                          Scenari Precaricati
                        </span>
                      </div>
                    </div>
        ...
        {activeTab === "study" && (
          <div>
            ...
            <div className="bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 sticky top-14 z-20 shadow-2xs">
              <div className="mx-auto max-w-5xl px-4 py-2.5 flex items-center gap-2 overflow-x-auto">
                <span className="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-wider whitespace-nowrap mr-1 hidden md:inline">
                  Blocchi:
                </span>
                {[
                    ...
                    <button
                      ...
                      className={`px-3 py-1.5 rounded-xl text-xs font-bold whitespace-nowrap transition cursor-pointer flex items-center gap-1.5 shrink-0 ${
                        isActive
                          ...
                          : "bg-slate-100 dark:bg-slate-700 text-slate-700 dark:text-slate-200 hover:bg-slate-200 dark:hover:bg-slate-600 hover:text-slate-900 dark:hover:text-white"
                      }`}
                    >
                      <span
        ...
        {activeTab === "stats" && (
          ...
          >
              <StatsHistoryView
                ...
                onStartTopicQuiz={handleStartTopicQuiz}
                onStartBlockQuiz={handleStartBlockQuiz}
              />
          </Suspense>
        )}
